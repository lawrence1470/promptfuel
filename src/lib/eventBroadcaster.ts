...
class EventBroadcaster extends EventEmitter {
	...
	addConnection(sessionId: string, connection: SSEConnection): void {
		...
		if (sessionConnections.size > 0) {
			console.log(`[EventBroadcaster] Cleaning up existing connections before adding new one for session: ${sessionId}`);
			this.cleanupSessionDeadConnections(sessionId);
		}
		
		sessionConnections.add(connection);
		
		console.log(`[EventBroadcaster] Added connection for session: ${sessionId}, total: ${sessionConnections.size}`);
	...
	removeConnection(sessionId: string, connection: SSEConnection): void {
		...
		console.log(`[EventBroadcaster] Removed connection for session: ${sessionId}`);
	}

	/**
	 * Send an event to all connections for a specific session
	 */
	sendToSession(sessionId: string, event: Omit<BuildEvent, "sessionId" | "timestamp">): boolean {
		const sessionConnections = this.connections.get(sessionId);
		if (!sessionConnections || sessionConnections.size === 0) {
			console.log(`[EventBroadcaster] No connections for session: ${sessionId}`);
			return false;
		}
		...
		console.log(`[EventBroadcaster] Broadcasting to session ${sessionId}:`, fullEvent);
	 ...
	 */
	private sendToConnection(connection: SSEConnection, event: BuildEvent): boolean {
		try {
	...
	private cleanupSessionDeadConnections(sessionId: string): void {
		...
		console.log(`[EventBroadcaster] Checking ${connections.size} connections for session: ${sessionId}`);

		for (const connection of connections) {
			const timeSinceLastHeartbeat = now - connection.lastHeartbeat;
			console.log(`[EventBroadcaster] Connection age: ${timeSinceLastHeartbeat}ms for session: ${sessionId}`);
			...
			if (timeSinceLastHeartbeat > 30000) {
				...
				try {
					...
					console.log(`[EventBroadcaster] Ping successful for session: ${sessionId}`);
				} catch (error) {
					// Connection is dead
					console.log(`[EventBroadcaster] Found dead connection for session: ${sessionId}`, error);
					deadConnections.push(connection);
				}
			}
		}
		...
		deadConnections.forEach(conn => {
			console.log(`[EventBroadcaster] Removing dead connection for session: ${sessionId}`);
			this.removeConnection(sessionId, conn);
		});
	}
	...
	forceCleanupSession(sessionId: string): void {
		...
		console.log(`[EventBroadcaster] Force cleaning up ${connections.size} connections for session: ${sessionId}`);
