...
export function useServerSentEvents({
  ...
  const connect = useCallback(() => {
    if (
      ...
      console.log(
        `[SSE] Skipping connection - sessionId: ${sessionId}, mounted: ${isMountedRef.current}, connecting: ${isConnecting}`
      );
      return;
    }
    ...
    if (eventSourceRef.current?.readyState !== EventSource.CLOSED) {
      console.log(
        `[SSE] Connection already exists for session: ${sessionId}, readyState: ${eventSourceRef.current?.readyState}`
      );
      return;
    }

    // Rate limit connection attempts (max 1 per 500ms for faster initial connection)
    const now = Date.now();
    const lastConnectAttempt = connectAttemptRef.current;
    if (lastConnectAttempt && now - lastConnectAttempt < 500) {
      console.log(
        `[SSE] Rate limiting connection attempt for session: ${sessionId}`
      );
      return;
    }
    ...
    try {
      ...
      console.log(`[SSE] Creating EventSource for session: ${sessionId}, URL: ${sseUrl}`);
      
      // Add timestamp to prevent caching issues
      const eventSource = new EventSource(`${sseUrl}?t=${Date.now()}`);
      eventSourceRef.current = eventSource;

      console.log(
        `[SSE] EventSource created, initial readyState: ${eventSource.readyState}`
      );
      
      // Add readyState monitoring
      const checkReadyState = () => {
        console.log(`[SSE] EventSource readyState check: ${eventSource.readyState} (0=CONNECTING, 1=OPEN, 2=CLOSED)`);
      };
      ...
      eventSource.onmessage = (event) => {
        try {
          ...
          if (!isConnected) {
            console.log(
              `[SSE] Setting connected state from message for session: ${sessionId}`
            );
            setIsConnected(true);
            setIsConnecting(false);
            setError(null);
      ...
      eventSource.onerror = (event) => {
        ...
        if (target.readyState === EventSource.CLOSED) {
          errorMessage = "Connection closed - checking server...";
        ...
        if (
          ...
          if (isServerError) {
            ...
            backoffDelay = Math.min(
              30000 * Math.pow(2, reconnectCountRef.current - 1),
              300000
            );
          } else {
          ...
          console.log(
            `[SSE] Reconnecting... attempt ${
              reconnectCountRef.current
            }/${reconnectAttempts} in ${backoffDelay}ms (${
          ...
          console.log(
            `[SSE] Not reconnecting for session: ${sessionId} (attempts: ${reconnectCountRef.current}, shouldRetry: ${shouldRetry})`
          );
  ...
  const sendMessage = useCallback((message: any): boolean => {
    ...
    console.warn(
      "[SSE] sendMessage not supported in SSE mode. Use HTTP requests for client->server communication."
    );
    return false;
  }, []);
  ...
  useEffect(() => {
    ...
    if (sessionId && sessionId !== "__INVALID__") {
      ...
      // Connect immediately to receive real-time updates
      console.log(
        `[SSE] useEffect executing immediate connect for session: ${sessionId}`
      );
      connect();
      ...
      console.log(`[SSE] useEffect skipped - invalid sessionId: ${sessionId}`);
    }
